...
extension View {
    ...
    /// and never covers pixels of the player.
    public func auroraFrame(cornerRadius: CGFloat, lineWidth: CGFloat = 1.5) -> some View {
        modifier(AuroraFrameModifier(cornerRadius: cornerRadius, lineWidth: lineWidth))
    }
...
private struct AuroraFrameModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .padding(lineWidth)
            .background(AuroraGradient.vertical, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }
}
