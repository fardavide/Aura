...
`.ai/skills/<name>/SKILL.md`; `.claude/skills`, `.agents/skills`, and `.gemini/skills` are native
...
Project skills (`.ai/skills/<name>/SKILL.md`, invoked through the active tool's skill mechanism):
...
specifics. `build-test` is the build/test workflow skill. `.claude/` retains Claude-specific
configuration; `.ai/` is the shared agent harness.
